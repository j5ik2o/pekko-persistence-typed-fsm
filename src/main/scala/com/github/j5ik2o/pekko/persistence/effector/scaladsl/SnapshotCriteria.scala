...
/**
 * スナップショット戦略を定義するトレイト スナップショットを取得すべきかどうかを判断するための条件を表す
 */
...
object SnapshotCriteria {
   ...
   *   条件のリスト
   * @param requireAll
   *   trueの場合はすべての条件を満たす必要がある（AND条件）、falseの場合はいずれかの条件を満たせばよい（OR条件）
   */
